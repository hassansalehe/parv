...
//          INS_RewriteMemoryOperand(ins, 0, REG(REG_INST_G0+0));
            ...
            // printf("temp (INS_IsMemoryRead(ins))
            // UNHANDLED MEMORY UNHANDLED MEMORY UNHANDLED MEMORY
            // op1:%s  op2:%s\n", op1, op2); fflush(stdout);
            // A special memory read operation. It may relate to stack
            // read operations
        }

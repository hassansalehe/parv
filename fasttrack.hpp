...
/************************************************/
static void ThreadCreate(void* addr,
                         ADDRINT parent_process_id,
                         ADDRINT my_process_id,
                         THREADID tid) {
    THREADID t     = tid;
    unsigned int u = t;
...
static void ThreadJoin(ADDRINT addr,
                       ADDRINT parent_process_id,
                       THREADID tid) {
...
static bool READ_RACE_NOT_OCCURS(ADDRINT ea, THREADID tid) {
    ...
    bool not_racy  = true;
    VCPOINTER Rx   = get_vector_clock(&glbR, x); // get read v.clocks
    ...
    //    if (!Rx->isRacy) {
        ...
        if (! vc_leq_epoch(Wx, Ct)) {
          // if (x.W > t.C[TID(x.W)]) error;     // write-read race
...
inline bool vc_epoch_eq(VCPOINTER vp1, Epoch e2) {
    ...
    assert(is_epoch(e2));
    return ((vc_epoch_tid(vp1->vc) == vc_epoch_tid(e2))
           && (vc_epoch_clock(vp1->vc) == vc_epoch_clock(e2)));
}
